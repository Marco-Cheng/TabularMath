# augmentation/verifier.py
from __future__ import annotations
from typing import Any, Dict, Tuple, Optional
import os, sys, math, random, multiprocessing as mp, traceback

# ===== 默认限制（可用环境变量覆盖）=====
TIME_LIMIT_SEC = int(os.getenv("GSMK_AUG_TIME_SEC", "300"))              # 5 分钟墙钟
MEM_LIMIT_BYTES = int(os.getenv("GSMK_AUG_MEM_BYTES", str(2 * 1024**3)))  # 2 GiB 虚拟内存
...
def compile_verifier(code: str):
    ...
    def _verifier_fn(assign: Dict[str, Any]) -> Tuple[bool, Any]:
        ok, res, err = _run_in_subproc(
            ...
            time_limit=TIME_LIMIT_SEC, mem_limit=MEM_LIMIT_BYTES
        )
        if not ok:
...
def compile_generator(code: str):
    """
